 ...
 * Когда id меняется в игре (см. `common/src/cards/idMigrations.ts` в fosord),
 * арт надо переименовать здесь — иначе картинка просто перестанет находиться.
 *
 ...
 * png с нуля, поэтому оставленный там `crusader.png` вернёт старый ключ (и
 * старый файл) в следующем же прогоне `bun run start`.
 *
 * Использование:
 *   bun run rename-ids.ts <map.json> [--dir docs] [--ext webp] [--dry-run]
 *
 * `map.json` — плоский объект `{"старый_id": "новый_id"}`; ровно тот формат, в
 * котором лежит `CARD_ID_MIGRATIONS`. Переименование идёт `git mv`, если папка
